...
public class BottleTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final SidedInvWrapper handler = new SidedInvWrapper(this, null);
	public int blockstateNew = this.getBlockState().getValue(BottleBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(BottleBlock.BLOCKSTATE);

	public BottleTileEntity(BlockPos pos, BlockState state) {
		super(WorldExpansionModBlockEntities.BOTTLE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(BottleBlock.BLOCKSTATE);
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<BottleTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<BottleTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public Component getDefaultName() {
		return Component.literal("bottle");
	}
