...
public class HalloweenDarkOakScarecrow0Feature extends Feature<NoneFeatureConfiguration> {
	...
	public HalloweenDarkOakScarecrow0Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("world_expansion", "halloween_dark_oak_scarecrow0"));
		if (template == null)
			return false;
