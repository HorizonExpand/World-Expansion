...
public class HalloweenDarkOakScarecrow1Feature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(ResourceKey.create(Registries.DIMENSION, new ResourceLocation("world_expansion:spookyfest_world")));
	private StructureTemplate template = null;

	public HalloweenDarkOakScarecrow1Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("world_expansion", "halloween_dark_oak_scarecrow1"));
		if (template == null)
			return false;
