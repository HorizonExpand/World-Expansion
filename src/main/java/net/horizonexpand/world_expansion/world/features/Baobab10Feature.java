...
import net.horizonexpand.world_expansion.procedures.Baobab10DopProcedure;

import java.util.Set;
import java.util.List;

public class Baobab10Feature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public Baobab10Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("world_expansion", "baobab10"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 175000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!Baobab10DopProcedure.execute(world, x, y, z))
					continue;
