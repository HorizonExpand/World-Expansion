...
import net.horizonexpand.world_expansion.procedures.Baobab7DopolnitielnoieUsloviieGienieratsiiProcedure;

import java.util.Set;
import java.util.List;

public class Baobab7Feature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public Baobab7Feature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("world_expansion", "baobab7"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 20000) {
			...
			for (int a = 0; a < count; a++) {
				...
				int y = spawnTo.getY();
				int z = spawnTo.getZ();
				if (!Baobab7DopolnitielnoieUsloviieGienieratsiiProcedure.execute(world, x, y, z))
					continue;
