...
public class MultiCraftingTableOutputSlotProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity instanceof Player _plrSlotItem && _plrSlotItem.containerMenu instanceof Supplier _splr && _splr.get() instanceof Map _slt ? ((Slot) _slt.get(9)).getItem() : ItemStack.EMPTY)
				.is(ItemTags.create(ResourceLocation.parse("world_expansion:copper_horns")))) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.smithing_table.use")), SoundSource.BLOCKS, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("block.smithing_table.use")), SoundSource.BLOCKS, 1, 1, false);
				}
			}
				...
				.is(ItemTags.create(ResourceLocation.parse("world_expansion:astilba_dye")))) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
