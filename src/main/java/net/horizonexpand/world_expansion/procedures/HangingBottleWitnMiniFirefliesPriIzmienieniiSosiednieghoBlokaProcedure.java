...
public class HangingBottleWitnMiniFirefliesPriIzmienieniiSosiednieghoBlokaProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (world.isEmptyBlock(BlockPos.containing(x, y + 1, z)) && world.getBlockState(BlockPos.containing(x, y - 1, z)).canOcclude()) {
			{
				...
				BlockState _bs = WorldExpansionModBlocks.BOTTLE_WITH_MINI_FIREFLIES.get().defaultBlockState();
		...
		} else if (world.isEmptyBlock(BlockPos.containing(x, y + 1, z)) && !world.getBlockState(BlockPos.containing(x, y - 1, z)).canOcclude()) {
			{
