...
import net.horizonexpand.world_expansion.procedures.TNTProjectileKoghdaSnariadPopadaietVBlokProcedure;
import net.horizonexpand.world_expansion.procedures.TNTProjectileKazhdyiTikPriPoliotieSnariadaProcedure;
...
public class TNTProjectileEntity extends AbstractArrow implements ItemSupplier {
	...
	public TNTProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(WorldExpansionModEntities.TNT_PROJECTILE.get(), world);
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		TNTProjectileKoghdaSnariadPopadaietVBlokProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		TNTProjectileKoghdaSnariadPopadaietVBlokProcedure.execute(this.level(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ(), this);
	}

	@Override
	public void tick() {
		super.tick();
		TNTProjectileKazhdyiTikPriPoliotieSnariadaProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}
	...
	public static TNTProjectileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		TNTProjectileEntity entityarrow = new TNTProjectileEntity(WorldExpansionModEntities.TNT_PROJECTILE.get(), entity, world);
	...
	public static TNTProjectileEntity shoot(LivingEntity entity, LivingEntity target) {
		TNTProjectileEntity entityarrow = new TNTProjectileEntity(WorldExpansionModEntities.TNT_PROJECTILE.get(), entity, entity.level());
